...
class TestRedisCommands(object):
    ...
    def test_zinterstore_max(self, r):
        ...
        r.zadd("c{foo}", a1=6, a3=5, a4=4)
        assert (
            r.zinterstore("d{foo}", ["a{foo}", "b{foo}", "c{foo}"], aggregate="MAX")
            == 2
        )
        assert r.zrange("d{foo}", 0, -1, withscores=True) == [
            (b("a3"), 5),
    ...
    def test_zinterstore_min(self, r):
        ...
        r.zadd("c{foo}", a1=6, a3=5, a4=4)
        assert (
            r.zinterstore("d{foo}", ["a{foo}", "b{foo}", "c{foo}"], aggregate="MIN")
            == 2
        )
        assert r.zrange("d{foo}", 0, -1, withscores=True) == [
            (b("a1"), 1),
    ...
    def test_zunionstore_max(self, r):
        ...
        r.zadd("c{foo}", a1=6, a3=5, a4=4)
        assert (
            r.zunionstore("d{foo}", ["a{foo}", "b{foo}", "c{foo}"], aggregate="MAX")
            == 4
        )
        assert r.zrange("d{foo}", 0, -1, withscores=True) == [
            (b("a2"), 2),
    ...
    def test_zunionstore_min(self, r):
        ...
        r.zadd("c{foo}", a1=6, a3=5, a4=4)
        assert (
            r.zunionstore("d{foo}", ["a{foo}", "b{foo}", "c{foo}"], aggregate="MIN")
            == 4
        )
        assert r.zrange("d{foo}", 0, -1, withscores=True) == [
            (b("a1"), 1),
