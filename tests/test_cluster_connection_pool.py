...
class TestBlockingConnectionPool(object):
    ...
    def test_repr_contains_db_info_unix(self):
        pool = redis.ConnectionPool(
            connection_class=redis.UnixDomainSocketConnection, path="abc", db=0
        )
...
class TestConnectionPoolUnixSocketURLParsing(object):
    def test_defaults(self):
        ...
        assert pool.connection_class == redis.UnixDomainSocketConnection
        assert pool.connection_kwargs == {"path": "/socket", "db": 0, "password": None}
    ...
    def test_db_as_argument(self):
        ...
        assert pool.connection_class == redis.UnixDomainSocketConnection
        assert pool.connection_kwargs == {"path": "/socket", "db": 1, "password": None}

    def test_db_in_querystring(self):
        pool = redis.ConnectionPool.from_url("unix:///socket?db=2", db=1)
        assert pool.connection_class == redis.UnixDomainSocketConnection
        assert pool.connection_kwargs == {"path": "/socket", "db": 2, "password": None}
...
class TestSSLConnectionURLParsing(object):
    ...
    def test_cert_reqs_options(self):
        ...
        pool = redis.ConnectionPool.from_url("rediss://?ssl_cert_reqs=optional")
        assert pool.get_connection("_").cert_reqs == ssl.CERT_OPTIONAL

        pool = redis.ConnectionPool.from_url("rediss://?ssl_cert_reqs=required")
